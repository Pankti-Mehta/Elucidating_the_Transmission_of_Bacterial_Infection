...
def construct_complete_weighted_digraph(gen_file: str, epi_file: str):
    list_for_patients = read_patient_sequences(gen_file)
    trace_tracker = read_patient_traces(epi_file)
    complete_wtd_digraph = {}
    # populating complete_wtd_digraph
    for pat in list_for_patients:
        complete_wtd_digraph[pat[0]] = {}
    max_e_val = float('-inf')
    for pat in trace_tracker:
        for pat_nbr in trace_tracker[pat]:
            if trace_tracker[pat][pat_nbr] > max_e_val:
                max_e_val = trace_tracker[pat][pat_nbr]
    dist_storer = compute_pairwise_gen_distances(list_for_patients, compute_genetic_distance)
    for patient in complete_wtd_digraph:
        for pat_nbr in complete_wtd_digraph:
            if patient != pat_nbr:
                graph_val = dist_storer[patient][pat_nbr]
                e_val = trace_tracker[patient][pat_nbr]
                complete_wtd_digraph[patient][pat_nbr] = graph_val + ((999 * (e_val / max_e_val)) / 100000)
    return complete_wtd_digraph
